...
class User(Base):
    ...
    capability_preferences: Mapped[list["CapabilityPreference"]] = relationship(
        back_populates="user",
        cascade="all, delete-orphan",
    )
...
class CapabilityPreference(Base):
    __tablename__ = "capability_preferences"
    __table_args__ = (UniqueConstraint("user_id", "slug", name="uq_user_capability_slug"),)
    ...
    user: Mapped[User] = relationship(back_populates="capability_preferences")
